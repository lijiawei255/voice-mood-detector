...
本模块提供针对不同情绪状态、压力水平和复合情绪的分层调节建议，
帮助用户了解自己的情绪状态并提供实用的自我调节方法。

建议体系架构：
1. 即时建议（TIPS）：按基础情绪类型分类
   - 愤怒、恐惧、悲伤、厌恶、惊讶：负面/中性情绪的即时缓解方法
   - 平静、开心：正向反馈和鼓励
   - 高压力：稳定度分数 >= 6.5 时的通用放松方法
2. 复合情绪建议（COMPOUND_TIPS）：对应 emotion_recognizer.py 中 COMPOUND_EMOTIONS
   - 焦虑、挫败、嫉妒、紧张、厌倦、愤怨：针对性的复合情绪缓解方法
3. 长期建议（LONG_TERM_TIPS）：稳定度 >= 3.5 时提供
   - 习惯培养、技能学习、生活方式调整等长期改善方案

建议选择策略（get_tips 函数）：
1. 复合情绪优先：检测到复合情绪时优先添加对应建议
2. 压力分级：稳定度分数 >= 6.5 时添加高压力类建议
3. 情绪匹配：添加对应基础情绪类型的建议
4. 正向反馈：平静/开心 + 压力不高时给予正面反馈
5. 兆底策略：确保始终能返回有意义的建议
6. 随机选取：避免每次显示相同内容，增强用户体验
...
# 复合情绪调节建议
# ===========================================================================
# 对应 emotion_recognizer.py 中定义的 COMPOUND_EMOTIONS 复合情绪模式
# 当复合情绪被检测到时，这些建议会优先于基础情绪建议被选取
# 每种复合情绪的建议针对其特定的心理机制设计
# ===========================================================================
COMPOUND_TIPS = {
    "焦虑": [
...
# 分层建议（长期建议）
# ===========================================================================
# 当情绪稳定度分数 >= 3.5 时提供长期调节建议
# 包括习惯培养、技能学习、生活方式调整等持续性改善方案
# 与即时建议配合使用，形成“即时缓解 + 长期改善”的完整建议体系
# ===========================================================================
LONG_TERM_TIPS = {
    "愤怒": [
